...
def x_y_normal_scan(GUI):
    ...
    while currentscan <= nbrofscans and stop_button['state'] == '!disabled':
        ...
        with open(folder + theday + '_' + filename + '_all_' + daily_scan_number + '_X.txt', 'a+') as f:
            data = np.loadtxt(f, dtype=float, delimiter='\t')
            f.truncate(0)
            np.savetxt(f, np.column_stack((data, datax)), fmt='%s', delimiter='\t')
        with open(folder + theday + '_' + filename + '_all_' + daily_scan_number + '_Y.txt', 'a+') as f:
            data = np.loadtxt(f, dtype=float, delimiter='\t')
            f.truncate(0)
            np.savetxt(f, np.column_stack((data, datay)), fmt='%s', delimiter='\t')

        # Plotting the data
        data_plot(delayps, xscansarray, GUI.fig_x_channel, GUI.toolbar_x)
        data_plot(delayps, yscansarray, GUI.fig_y_channel, '')
        data_plot(delayps, xscansarray, GUI.fig_fft, '')
    ...
    GUI.fig_x_channel.savefig(folder + theday + '_' + filename + '_mean_' + str(daily_scan_number) + '_X.png',
                              ...
                              transparent=True, bbox_inches='tight', frameon=None
                              )
...
def data_plot(x_data, y_data, figure, toolbar):
